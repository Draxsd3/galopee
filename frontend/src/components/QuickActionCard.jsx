...
/*
 * Cards de ação rápida da home. Cada variante define a cor que colore a imagem
 * de fundo por meio de um gradiente semitransparente, dando identidade visual
 * sem esconder completamente a foto.
 */
const VARIANTS = {
    greenLight: {
...
export default function QuickActionCard({
    ...
    return (
        <Link
            to={to}
            className={`group relative flex flex-col justify-between overflow-hidden rounded-[24px] p-6 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${heightClass}`}
        >
            {bgImage ? (
                <img
                    src={bgImage}
