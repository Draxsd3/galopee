...
export default function ProductDetail() {
    ...
    // Carrega avaliações locais (por produto)
    useEffect(() => {
    ...
    // Produtos relacionados (mesma categoria) e da mesma loja
    useEffect(() => {
        if (!product) return;
        let active = true;
        const jobs = [];
        if (product.category) {
            jobs.push(api.get('/products', { params: { category: product.category, limit: 10 } })
                .then((res) => ({ kind: 'related', items: res.data.items || [] })));
        }
        if (product.seller_id) {
            jobs.push(api.get('/products', { params: { sellerId: product.seller_id, limit: 10 } })
                .then((res) => ({ kind: 'sameStore', items: res.data.items || [] })));
        }
        Promise.all(jobs).then((results) => {
            if (!active) return;
            for (const r of results) {
                const filtered = r.items.filter((p) => p.id !== product.id).slice(0, 4);
                if (r.kind === 'related') setRelated(filtered);
                if (r.kind === 'sameStore') setSameStore(filtered);
            }
        }).catch(() => {
            if (!active) return;
            setRelated([]);
    ...
    const handleAdd = async () => {
        ...
        if (!['buyer', 'seller'].includes(user.role)) {
            toast.error('Este perfil não pode adicionar itens ao carrinho');
            return;
        }
        try {
            setAdding(true);
    ...
    if (loading) return <div className="container-page py-12 text-center text-slate-500">Carregando...</div>;
    if (!product) return (
    ...
    return (
        <div className="container-page py-8">
            ...
            {/* Bloco principal: imagem + informações */}
                ...
                <div className="space-y-5 text-center md:text-left">
                    ...
                    <div>
                        ...
                        <div className="mt-1 flex items-center justify-center gap-1 text-sm text-brand-700 md:justify-start">
                            ...
                            Em até 12x de <strong className="mx-1">{formatPrice(installmentValue)}</strong> sem juros
                        </div>
                    </div>

                    <p className="leading-relaxed text-slate-600">{product.description || 'Sem descrição disponível.'}</p>
                    ...
                    >
                        ...
                        {maxStock === 0 ? 'Indisponível' : adding ? 'Adicionando...' : 'Adicionar ao carrinho'}
                    </button>
                    ...
                    <div className="grid grid-cols-1 gap-3 border-t border-slate-100 pt-4 sm:grid-cols-3">
                        ...
                        <div className="flex items-center justify-center gap-2 text-sm text-slate-600 md:justify-start">
                            ...
                            {product.free_shipping ? 'Frete grátis' : 'Envio Brasil'}
                        </div>
                        <div className="flex items-center justify-center gap-2 text-sm text-slate-600 md:justify-start">
            ...
            {/* Bloco da loja / vendedor */}
            <section className="mt-10">
                <div className="card flex flex-col items-center gap-5 p-6 text-center md:flex-row md:items-center md:justify-between md:text-left">
                        ...
                        <div>
                            <div className="flex items-center justify-center gap-2 md:justify-start">
                                <span className="text-lg font-bold text-slate-900">
                                    {product.store_name || 'Loja Galoppe'}
                                </span>
                                {product.seller_verified && (
                                    <span className="inline-flex items-center gap-1 rounded-full bg-brand-50 px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-brand-700">
                                        <Check className="h-3 w-3" strokeWidth={3} />
                                        Verificada
                                    </span>
                                )}
                            </div>
                            <div className="mt-1 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-sm text-slate-500 md:justify-start">
                    ...
                    <dl className="grid gap-x-8 gap-y-4 sm:grid-cols-2">
                        ...
                        <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                            ...
                            <dd className="text-slate-800">
                                {product.free_shipping ? 'Frete grátis' : 'Calculado no checkout'}
                            </dd>
                        </div>
                        <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                            <dt className="font-semibold text-slate-500">Parcelamento</dt>
                            <dd className="text-slate-800">Até 12x sem juros</dd>
                        </div>
                        <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                            <dt className="font-semibold text-slate-500">Vendido por</dt>
                            <dd className="text-slate-800">{product.store_name || 'Galoppe'}</dd>
                        </div>
                    </dl>
                </div>
            </section>
            ...
            {sameStore.length > 0 && (
                <section className="mt-12">
                    <div className="mb-5 flex flex-col items-center justify-between gap-2 md:flex-row md:items-end">
                        <div className="text-center md:text-left">
                            ...
                            <h2 className="mt-2 text-2xl font-bold text-slate-900">
                                Mais produtos de {product.store_name}
                            </h2>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                        {sameStore.map((p) => <ProductCard key={p.id} product={p} />)}
                    </div>
                </section>
            )}

            {/* Recomendados (mesma categoria) */}
            {related.length > 0 && (
                <section className="mt-12">
                    <div className="mb-5 text-center md:text-left">
                        <span className="section-kicker">Recomendados para você</span>
                        <h2 className="mt-2 text-2xl font-bold text-slate-900">Produtos semelhantes</h2>
                        <p className="text-sm text-slate-500">Quem viu este, também viu.</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <section className="mt-12">
                ...
                >
                    ...
                    <textarea
                        ...
                        rows={3}
                        placeholder="Conte como foi a sua experiência com este produto..."
                ...
                {reviews.length === 0 ? (
                    <div className="card p-8 text-center text-slate-500">
                        Ainda não há avaliações para este produto.<br />
                        Seja o primeiro a comentar.
                    </div>
                ) : (
                    <ul className="space-y-4">
                        {reviews.map((r) => (
                            <li key={r.id} className="card p-5 text-center md:text-left">
